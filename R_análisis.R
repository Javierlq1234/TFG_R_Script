...
# Código completo para simular los datos de manera realista
...
# 3. ASIGNACIÓN ALEATORIA A GRUPOS DENTRO DE CADA BLOQUE (BLOQUEO)
...
# 5. SIMULAR NASA-TLX (variables continuas truncadas entre 0 y 100)
...
# Incluir también 'n_temas' en las variables del t-test
variables_ttest <- c(
  "NASA_total",
  "demanda_mental",
  "demanda_temporal",
  ...
  "rendimiento",
  "calidad_texto",
  "n_temas"  # Ahora se analiza con t-test
)

# Crear lista para guardar resultados
resultados_t <- list()

# Ejecutar t-test y d de Cohen para todas las variables
...
# Seleccionar las subescalas del NASA-TLX
nasa_items <- df_limpio[, c("demanda_mental", "demanda_temporal", "esfuerzo", "frustracion", "rendimiento")]

# Ejecutar PCA con 2 componentes, sin rotación
pca_result <- principal(nasa_items, nfactors = 2, rotate = "none")

# Ver resumen completo
print(pca_result)

# Cargas factoriales (qué tanto aporta cada subescala a cada componente)
cat("\nCargas factoriales de cada subescala:\n")
print(round(pca_result$loadings[, 1:2], 3))

# Varianza explicada
cat("\nVarianza explicada por cada componente (%):\n")
print(round(pca_result$Vaccounted[2, ], 2))
